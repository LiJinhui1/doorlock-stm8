...
/*Receive的例程*/
/*
1、唤醒是下降沿唤醒，然后4ms低电平，唤醒4ms后必须发送数据。
2、接收到开始信号后，每隔1s发送8字节数据(double类型) 分别是功率和电池电压,如果没有结束信号的话，坚持10s后关机
...
void Bus_SendByte(uint8_t* sendbuf,uint8_t size){
  TIM3_DeInit(); //由于发送数据的时候这个管脚有电平变化，所以不能使能TIM3
  ...
  while (1)
  {
    ...
    if(patience<5)
    {
      ...
      { 
        //ADC与DMA开始启动
        LED_ON;
        MEASURE_ON;       //开启Dset，测量模式有3us的建立时间
